...
packages<-c('dplyr','mice','vtreat','Amelia','caret','doParallel','foreach',
          'ggplot2','rms','parallel','pec','plotrix','prodlim','qs','ranger','survival','timeROC')
    ...
    for (n in 1:length(mice)){
      ...
      for (p in 1:length(deathtimes)){
        for (q in 1:nrow(datax[-folds[[t]],])){
          standerx[q,p]<-std.error(survfull[q,p,c(1:rsf1$num.trees)])
        }  
      }
      ...
      for (p in 1:length(deathtimes2)){
        for (q in 1:nrow(datax[folds[[t]],])){
          standerx2[q,p]<-std.error(survfull2[q,p,c(1:rsf1$num.trees)])
        }  
      }
